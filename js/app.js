...
async function initDualCalculator() {
    try {
        ...
        window._goldData = {
            ...
            sanaaUSD: d.regions?.sanaa?.currencies?.USD?.buy || 533,
            adenUSD: d.regions?.aden?.currencies?.USD?.buy || 1550
        };
        ...
        window.updateMetalCalculator = function() {
            ...
            if (type === 'silver') {
                // الفضة عالمية دائماً
                const silverOz = d.silver?.price_usd || 28.8;
                pricePerGram = silverOz / 31.1035;
                if (currency === 'YER' && activeTab === 'local') {
                    ...
                    pricePerGram *= usdRate;
                }
            } else if (activeTab === 'local') {
                // 🏠 محلي - استخدم أسعار السوق اليمني
                const region = document.getElementById('local-region')?.value || 'sanaa';
                const goldPrices = region === 'aden' ? window._goldData.aden : window._goldData.sanaa;
                const karat = type.replace('gold-', '') + 'k';
                pricePerGram = goldPrices?.[karat]?.buy || (region === 'aden' ? 19200 : 63203);
            } else {
                // 🌍 عالمي - استخدم سعر الذهب العالمي
                const goldOz = window._goldData.world.ounce;
                const gram24USD = goldOz / 31.1035;
                ...
                // تحويل العملة للعالمي
                if (currency === 'YER') pricePerGram *= (window._goldData.sanaaUSD || 533);
                else if (currency === 'SAR') pricePerGram *= (window._goldData.sanaaUSD / 3.75 || 140);
            }
            ...
            const resultEl = document.getElementById('metal-calc-result');
            if (resultEl) {
                const symbol = currency === 'YER' ? 'ر.ي' : currency === 'SAR' ? '﷼' : '$';
                resultEl.textContent = total.toLocaleString() + ' ' + symbol;
            }
        };
        ...
        document.querySelectorAll('.tab-btn').forEach(btn => {
            btn.addEventListener('click', () => {
                setTimeout(() => window.updateMetalCalculator(), 200);
            });
        });
        
        // حدث تغيير المنطقة
        document.getElementById('local-region')?.addEventListener('change', () => {
            if (document.querySelector('.tab-btn[data-tab="local"].active')) {
                window.updateMetalCalculator();
            }
        });
        
        window.updateMetalCalculator();
        console.log('✅ حاسبة مزدوجة جاهزة (عالمي + محلي)');
    } catch(e) {
        console.log('⚠️', e);
    }
}
